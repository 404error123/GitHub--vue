<template>
	<header>
		<div class="header-cnt clearfix">
			...
			<div class="search">
				<el-autocomplete popper-class="my-autocomplete" v-model="state3" :fetch-suggestions="querySearch" placeholder="Search or jump to..."
				 @select="handleSelect">
			...
			<div class="sign-in">
				<ul>
					<li>
						<el-tooltip v-if="newMess" effect="dark" content="You have unread notifications" placement="bottom">
							<a href="" class="bell mess">
								<i class="el-icon-bell"></i>
							</a>
						</el-tooltip>
						<a v-else href="javascript:;" class="bell">
							<i class="el-icon-bell"></i>
						</a>
					</li>
					<li>
					...
					<li>
						<a href="javascript:;" v-if="isLogin">
							<el-dropdown trigger="click">
								<span class="el-dropdown-link">
									<span class="head-ico">
										<img v-if="user" :src="user.imageUrl" alt="">
									</span>
									<i class="el-icon-caret-bottom"></i>
								</span>
								<el-dropdown-menu slot="dropdown">
									...
									<el-dropdown-item>
                                        <router-link :to="{ name: 'login' }" @click.native="signOut">Sign out</router-link>
                                    </el-dropdown-item>
								</el-dropdown-menu>
							</el-dropdown>
						</a>
                        <router-link :to="{ name: 'login' }" class="login" v-else>Sign in</router-link>
					</li>
				</ul>
			</div>
		</div>
...
export default {
    ...
    methods: {
        ...
        createFilter(queryString) {
            return text => {
                return (
                    text.toLowerCase().indexOf(queryString.toLowerCase()) === 0
                );
            };
        },
        handleSelect(item) {
        ...
        signOut() {
            this.$axios.get('http://127.0.0.1:5000/signout').then(() => {
...
header .header-cnt .sign-in a.login {
    font-size: 14px;
    line-height: 40px;
}
.el-dropdown-menu li {
...
.el-dropdown-menu li:not(.user-name):hover a {
    color: #66b1ff;
}
</style>